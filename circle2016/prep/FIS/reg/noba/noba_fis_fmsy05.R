...
catch<- read.csv('circle2016/prep/FIS/reg/noba/spatial_catch_prebbmsy_fmsy05.csv')%>%
...
saveRDS(cmsy_fits, file = file.path(fis_dir,"reg/noba/catch_model_bmsy_noba/cmsy-fits-fmsy05.rds"))
...
write.csv(cmsy_bbmsy, "circle2016/prep/FIS/reg/noba/catch_model_bmsy_noba/cmsy_bbmsy_noba_fmsy05.csv", row.names=FALSE)
...
cmsy <- read.csv('circle2016/prep/FIS/reg/noba/catch_model_bmsy_noba/cmsy_bbmsy_noba_fmsy05.csv') %>%
...
new_b_bmsy <- function(b_bmsy=constrained, method = "cmsy"){
  ...
  write.csv(b_bmsy, sprintf('circle2016/prep/FIS/reg/noba/meanbmsy/%s_b_bmsy_%s_mean5yrs_noba_fmsy05.csv', method, unique(b_bmsy$prior)), row.names=FALSE)
}
...
cmsy <- read.csv('circle2016/prep/FIS/reg/noba/meanbmsy/cmsy_b_bmsy_constrained_mean5yrs_noba_fmsy05.csv') %>%
...
mean_catch <- read.csv("circle2016/prep/FIS/reg/noba/fmsy05_meancatch.csv") %>%
...
write.csv(data, file='circle2016/prep/FIS/reg/noba/fis_cmsy_bbmsy_noRAM_noba_fmsy05.csv', row.names=FALSE)
...
FIS = function(layers, status_year){
  ...
  #catch data
  c<- read.csv('circle2016/prep/FIS/reg/noba/fmsy05_meancatch.csv') %>%
    dplyr::select(
      rgn_id,
      stock_id_taxonkey,
      year,
  ...
  # b_bmsy data
  b<- read.csv('circle2016/prep/FIS/reg/noba/fis_cmsy_bbmsy_noRAM_noba_fmsy05.csv') %>%
    dplyr::select(
      rgn_id,
      stock_id,
      year,
...
write.csv(status_data, 'circle2016/prep/FIS/reg/noba/status/fmsy05_status.csv')
